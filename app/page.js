...
const PRODUCTS = [
  ...
  {id:"lemon-cake",name:"老奶奶檸檬糖霜蛋糕",image:"/products/lemon-cake.jpeg",sizes:[{label:"6 吋",price:380}],ingredients:["檸檬蛋糕","檸檬糖霜"]},
  {id:"taro-flower",name:"芋泥小花",vegetarian:true,image:"/products/taro-flower.jpeg",sizes:[{label:"6 吋",price:780},{label:"8 吋",price:1180},{label:"10 吋",price:1780},{label:"12 吋",price:2100}],ingredients:["原味戚風蛋糕","香甜芋泥","滑嫩布丁"]},
  ...
  {id:"blueberry-chocolate-smile",name:"藍莓巧克力難哄",vegetarian:true,image:"/products/blueberry-chocolate-smile.jpeg",sizes:[{label:"4 吋",price:680},{label:"6 吋",price:980},{label:"8 吋",price:1280}],ingredients:["可可戚風蛋糕","手熬藍莓果醬","滑嫩布丁"]},
  ...
  {id:"fruit-naked",name:"水果裸蛋糕",vegetarian:true,image:"/products/fruit-naked.jpeg",sizes:[{label:"5 吋",price:798},{label:"7 吋",price:1288}],ingredients:["原味戚風蛋糕","當季水果","香緹鮮奶油","香草外交官奶醬"]},
...
const defaultSettings = {
  ...
  bankNote:"匯款完成後，請加入 LINE 官方帳號並提供訂購人姓名與帳號後五碼。", lineUrl:"", serviceHours:"每日 10:00–20:00", announcement:"",
...
export default function Home(){
  ...
  return <>
    ...
    <main id="top">
      ...
      <section id="products" className="section products"><p className="eyebrow">OUR DESSERTS</p><h2>商品一覽</h2><p>商品頁單純呈現照片、尺寸、價格與實際內容物。</p><div className="product-grid product-grid-photo">{PRODUCTS.filter(p=>!p.custom).map(p=><ProductCard key={p.id} p={p} onChoose={chooseProduct}/>)}</div></section>
      ...
      <section className="section faq" id="faq"><p className="eyebrow">ORDER INFORMATION</p><h2>訂購與取貨</h2><div className="faq-grid"><details open><summary>取貨方式</summary><p>以門市自取為主。若有需要，可協助安排 Lalamove 配送，運費由顧客自行負擔。</p></details><details><summary>送出表單就代表訂單成立嗎？</summary><p>尚未。須由初甜趣透過 LINE 或電話確認後，訂單才正式成立。</p></details><details><summary>客製蛋糕多久前預訂？</summary><p>客製道具需至少提前 14～30 個工作天預訂，並先透過官方 LINE 討論。</p></details><details><summary>付款方式</summary><p>可選擇銀行轉帳或取貨當天現金付款。</p></details></div></section>
    ...
    {open&&<div className="modal" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><div className="dialog"><button className="x" onClick={()=>setOpen(false)}>×</button><p className="eyebrow">ORDER FORM</p><h2>填寫訂購資料</h2><p className="selected-date">取貨日期：{selected}</p><form onSubmit={submit}>
      ...
      {payment==="bank"&&<div className="bank-card"><h3>銀行匯款資訊</h3><p><b>銀行：</b>{settings.bankName}</p><p><b>代碼：</b>{settings.bankCode}</p><p><b>帳號：</b>{settings.bankAccount}</p><button type="button" className="copy-bank" onClick={()=>navigator.clipboard?.writeText(settings.bankAccount)}>複製帳號</button><small>{settings.bankNote}</small></div>}
      <label>蛋糕文字／蠟燭／盤叉／其他備註<textarea name="note" rows="4"/></label><label className="agree"><input type="checkbox" required/>我了解送出後仍須由店家確認，才算正式成立訂單。</label><button className="primary submit" disabled={sending}>{sending?"傳送中…":"送出訂單"}</button>{message&&<div className="result"><p>{message}</p>{orderId&&<p><b>訂單編號：{orderId}</b></p>}</div>}
    </form></div></div>}
  </>
}
