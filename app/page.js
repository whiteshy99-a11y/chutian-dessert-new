...
export default function Home(){
  ...
  return <>
    <header><a className="brand brand-text" href="#top"><span className="brand-title">✿ 初甜趣</span><span className="brand-subtitle">HANDMADE DESSERT · SINCE 2024</span></a><nav><a href="#calendar">可訂日期</a><a href="#products">商品</a><a href="#custom">客製蛋糕</a><a href="#about">品牌故事</a><a href="#contact">聯絡我們</a></nav></header>
    <main id="top">
      ...
      <section className="section faq" id="faq"><p className="eyebrow">ORDER INFORMATION</p><h2>訂購與取貨</h2><div className="faq-grid"><details><summary>取貨方式</summary><p>門市自取為主，如有配送需求，可協助安排 Lalamove，運費由顧客自行負擔。</p></details><details><summary>送出表單就代表訂單成立嗎？</summary><p>送出訂單後，請加入 LINE 客服提供訂單編號、上傳付款截圖，經店家確認後訂單才會成立。</p></details><details><summary>客製蛋糕多久前預訂？</summary><p>客製道具需至少提前 14～30 個工作天預訂，並先透過官方 LINE 討論主題、造型與報價。</p></details><details><summary>付款方式</summary><p>銀行轉帳匯款</p></details><details><summary>訂購流程</summary><p>Step 1　填寫訂購資料並送出訂單<br/>Step 2　加入官方 LINE 客服<br/>Step 3　傳送訂單編號完成綁定<br/>Step 4　上傳匯款資訊（截圖），待店家確認後訂單成立</p></details></div></section>
    ...
    {open&&<div className="modal" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><div className="dialog"><button className="x" onClick={()=>setOpen(false)}>×</button>{orderId?<div className="order-success"><p className="success-icon">✓</p><p className="eyebrow">ORDER RECEIVED</p><h2>訂單已送出</h2><p>您的訂單資料已收到，請完成以下步驟。</p><div className="order-number"><span>訂單編號</span><strong>{orderId}</strong></div><div className="success-steps"><p><b>1.</b> 點下方按鈕前往 LINE</p><p><b>2.</b> LINE 會自動帶入訂單編號，請按「傳送」完成綁定</p><p><b>3.</b> 綁定成功後直接上傳匯款截圖</p><p><b>4.</b> 經店家確認款項後，訂單才正式成立</p></div><div className="success-reminder"><b>提醒您</b><br/>不需要自己輸入訂單編號；進入 LINE 後只要把已帶入的訂單編號傳送出去即可。</div>{settings.lineUrl&&<a className="line-success" href={(()=>{const m=settings.lineUrl.match(/@[A-Za-z0-9._-]+/);return m?`https://line.me/R/oaMessage/${encodeURIComponent(m[0])}/?${encodeURIComponent(orderId)}`:settings.lineUrl})()} target="_blank" rel="noreferrer">前往 LINE 綁定此訂單 →</a>}</div>:<><p className="eyebrow">ORDER FORM</p><h2>填寫訂購資料</h2><p className="selected-date">取貨日期：{selected}</p><form onSubmit={submit}>
