...
export function summarizeWebsiteMonth(orders, month) {
  const paid = (orders || []).filter((order) => {
    ...
    return taipeiMonth(paidAt) === month;
  });
  const revenue = paid.reduce((sum, order) => sum + getOrderAmount(order), 0);
  const unpriced = paid.filter((order) => getOrderAmount(order) <= 0).map((order) => order.orderId);
  return { revenue, count: paid.length, unpriced };
}
